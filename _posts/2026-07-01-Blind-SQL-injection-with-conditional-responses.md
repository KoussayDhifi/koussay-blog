...
When we use Burp Suite and intercept the traffic when we look for a specific category, we find an interesting field in the headers, specifically in the cookies field, which is the `TrackingId`, shown in the following image.
